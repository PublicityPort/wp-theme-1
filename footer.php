				...
				<div class="site-info">
					<?php if(! empty( $options['copyright_text'] ) ) { 
			       		echo wp_kses_post( $options['copyright_text'] ); 
			        	        ...
			        	        printf( esc_html__( '| Powered by %1$s | Web app by %2$s', 'daily-insight'), '<a href="'.esc_url( 'https://wordpress.org/' ).'">WordPress</a>', '<a href="'. esc_url( 'https://publicityport.com/?utm_source=stb&utm_medium=footer-link' ) .'">Publicity Port</a>' ); ?>
				</div><!-- .site-info -->
			</div><!-- .bottom-footer -->
		</div><!-- .container -->
	</footer><!-- #colophon -->
